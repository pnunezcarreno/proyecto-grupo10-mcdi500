...
* **`19-06 Commit: 8f82923`** Se clona directorio `F3/src/` que contiene las clases de preprocesamiento y el motor analítco, en el directorio `F4/src/`. Esto garantiza la reproducibilidad del notebook de la Fase 4 al no depender de rutas de fases anteriores.
...
* **`17-06 Commit: 32037b2`** Implementación del Patrón *Strategy* utilizando Clases (ej. `EscaladorMinMax`, `FiltroIPs`) para lograr alta cohesión y bajo acoplamiento.
* **`17-06 Commit: 32037b2`** Reestructuración del código del proyecto utilizando el paradigma de Programación Orientada a Objetos (POO) para mejorar su organización, modularidad y escalabilidad. El código se almacena en `F3/src/preprocesamiento.py`.
...
### V3.0
##### Fase 3 - Semana 2 - Junio 2026
* **`14-06 Commit: 933a636`** Implementación de comparación de los algoritmos iterativo $O(n^2)$ y recursivo (*Merge Sort* $O(n \log n)$) para demostrar cuál de los dos es el más apto para el contexto de análisis de eventos de ciberseguridad. La lógica del motor analítico se encapsula en `F3/src/motor_analisis.py`.
